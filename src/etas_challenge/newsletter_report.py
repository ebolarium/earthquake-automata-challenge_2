...
def render_daily_report(
    ...
    labels = {
        ...
        "result": "Result" if english else "Sonuç",
    ...
    for score in scores:
        ...
        score_rows += (
            ...
            f"<td>{_number(score['mean_igpe'])}</td><td>{winner}</td></tr>"
        )
    latest_block = (
        f"<h2 style=\"font-size:15px;margin:26px 0 8px\">{labels['latest']} · {target_date}</h2>"
        f"<table style=\"width:100%;border-collapse:collapse;font-size:12px\"><tbody>{score_rows}</tbody></table>"
...
<p style="margin:28px 0"><a href="{html.escape(dashboard_url, quote=True)}" style="padding:10px 14px;background:#087f7a;color:#fff;text-decoration:none;border-radius:4px;font-weight:700;font-size:12px">{labels['dashboard']}</a></p>
